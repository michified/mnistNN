...
**A from-scratch implementation achieving ~93% accuracy on eMNIST**

---

## 🧠 Model Architecture
- **Input Layer**: 784 input neurons for the 28x28 image size for the eMNIST dataset
...
- **Prediction demo** written in python (classifier.py) which classifies your hand-drawn digit

---

## 📊 Statistics (ran on 13th Gen Intel(R) Core(TM) i7-13700K)
- **Neural Net Creation**: 5 ms
- **Training Time**: 2518 seconds
...
- **Data Augmentation**: Rotate the eMNIST dataset arbitrarily for better prediction performance
...
- **Demo Improvement**: Clean the input in classifier.py better to accomodate for smaller drawings
...
3. Compile the training program:
   ...
   ```
4. (Optional) Run the training program to train the model:
   ```bash
   ./train.exe
   ```
